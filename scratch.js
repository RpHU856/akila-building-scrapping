...
async function renderMapSelection(res) {
    ...
    const offset = 0.004;
    const initialBbox = `${parseFloat(res.lon) - offset},${parseFloat(res.lat) - offset},${parseFloat(res.lon) + offset},${parseFloat(res.lat) + offset}`;
...
async function runMultiScan() {
    ...
    const entree = $("input-entree").value.trim() || Array.from(selectedRnbIds)[0];
